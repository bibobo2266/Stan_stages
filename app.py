...
def load_ranking(token: str) -> pd.DataFrame:
    """Whole-market market cap for ranking. One call, most recent day."""
    api = DataLoader(); api.login_by_token(api_token=token)
    for back in range(0, 10):
        ...
        try:
            ...
            df = pd.DataFrame()
        if not df.empty:
            return df[["stock_id", "market_value"]]
    return pd.DataFrame()
...
with st.sidebar:
    ...
    rank_by = st.selectbox(
        "掃描範圍（取前 N 大）", ["市值 (大→小)", "代號順序"],
        help="市值=先掃台股最大的 N 檔，最實用。動能/RS 掃完後可再排序。")
...
if go:
    ...
    if rank_by != "代號順序":
        rk = load_ranking(token)
        if not rk.empty:
            pool = pool.merge(rk, on="stock_id", how="left")
            pool = pool.sort_values("market_value", ascending=False, na_position="last")
        else:
            st.warning("市值資料抓取失敗，改用代號順序。")
    ...
    # Stage 2 must beat the index: drop laggards (RS<0)
