...
def build(liq: float, years: int = S.YEARS_DEFAULT, as_of=None):
    P, uni = S.load_frames(min_wan=liq, years=years, as_of=as_of)
...
with st.sidebar:
    ...
    rewind = st.checkbox("回到某一天重算", value=False,
                         help="把資料截斷到指定日期，等同回到那一週跑一次。"
                              "階段判斷用週線，所以週一到週四結果相同——"
                              "選週內任一天，拿到的都是上週五收盤的判斷。")
    as_of = None
    yrs = S.YEARS_DEFAULT
    if rewind:
...
<b>資料</b> 還原股價，2015-06 起（台股漲跌幅放寬到 10% 的起點）。母體含當時<br>
實際在交易的股票，包括後來下市的——用今天的名單回頭掃會有倖存者偏差。
</div>
""", unsafe_allow_html=True)
    st.stop()

d, ms, mbias, wk, partial, pool_n = build(liq, yrs, as_of)
